...
void LandingMark::processImage()
{
	...
	//cvtColor(image, img_aux, COLOR_BGR2Lab);
...
void LandingMark::processImage(int hsv_min_blue[3], int hsv_max_blue[3], int hsv_min_yellow[3], int hsv_max_yellow[3])
{
	Mat img_aux;
	...
	// Pega a area azul
	img_blue_C1 = imlimiares(main_imgHSV_C3, hsv_min_blue, hsv_max_blue);
	bitwise_and(main_imgHSV_C3, main_imgHSV_C3, hsv, img_blue_C1);

	// Pega a area amarela
	img_yellow_C1 = imlimiares(main_imgHSV_C3, hsv_min_yellow, hsv_max_yellow);
	bitwise_and(hsv, hsv, output, img_yellow_C1);
...
Mat LandingMark::rotatedToImage()
{
	...
	//M = getRotationMatrix2D(markRotatedRect.center, angle, 1.0);

	//warpAffine(main_imgHSV_C3, rotated, M, main_imgHSV_C3.size(), INTER_CUBIC);

	// crop the resulting image
	//getRectSubPix(rotated, rect_size, markRotatedRect.center, cropped);
